...
bool SimpleNonsequentialFinder::find_best(primitives::length_t cost)
{
    m_cost = cost;
    m_next_cost = std::numeric_limits<decltype(m_next_cost)>::max();
    reset_search();
...
void SimpleNonsequentialFinder::start_search(const primitives::point_id_t swap_start
...
void SimpleNonsequentialFinder::delete_edge(const primitives::length_t removed
...
bool SimpleNonsequentialFinder::feasible() const
{
...
void SimpleNonsequentialFinder::add_edge(const primitives::point_id_t new_start
    ...
    , const primitives::length_t added)
{
    ...
    if (not improving and not feasible())
    {
        const auto cost = total_closing_add - total_remove;
        if (cost > m_cost)
        {
            m_next_cost = std::min(cost, m_next_cost);
        }
        else if (cost == m_cost)
        {
            // check if recreating existing edge.
            if (new_start != m_tour.prev(m_swap_end) and new_start != m_tour.next(m_swap_end))
            {
                ...
                // TODO: 2-opt does not require execution of swap.
                test_tour.multicycle_swap(m_starts, m_ends, m_removes);
                if (test_tour.cycles() == 1)
                {
                    std::cout << __func__ << ": error: tour not split." << std::endl;
                }
                else
                {
                    ...
                    {
                        Merger merger;
                        merger.merge_once(m_root, test_tour, cost, true);
