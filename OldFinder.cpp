    ...
    , const primitives::length_t added)
{
    ...
    if (improving)
    {
        if (new_start != m_tour.prev(m_swap_end) and new_start != m_tour.next(m_swap_end))
        {
            ...
            {
                if (m_save_nonsequential)
                {
                    m_nonsequential_moves.push_back({m_starts, m_ends, m_removes});
                }
                //std::cout << "\nnonsequential improvement: " << improvement << std::endl;
                auto test_tour = m_tour;
                test_tour.multicycle_swap(m_starts, m_ends, m_removes);
                if (test_tour.cycles() < 2)
                {
                    print_move();
                    std::cout << __func__ << ": error: multicycle swap resulted in only "
                        << test_tour.cycles() << " cycles." << std::endl;
                    std::abort();
                }
                if (test_tour.min_cycle_size() > 1)
                {
                    /*
                    std::cout << "cycles created; max outgroup length, cycles: "
                        << test_tour.max_outgroup_length()
                        << ", " << test_tour.cycles() << std::endl;
                    std::cout << "first cycle:" << std::endl;
                    test_tour.print_first_cycle();
                    */
                    Merger merger;
